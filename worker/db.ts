  ...
  if (user.role === "admin") {
    return (
      await c.env.DB.prepare(
        "SELECT id, public_id AS publicId, name, CASE WHEN hero_data IS NULL OR hero_data = '' THEN NULL ELSE '/api/shops/' || public_id || '/hero?v=' || COALESCE(hero_hash, 'original') END AS heroUrl, latitude, longitude, radius_meters, created_by FROM shops ORDER BY created_at DESC",
      ).all<ShopRow>()
    ).results;
  }
  return (
    await c.env.DB.prepare(
      `SELECT shops.id, shops.public_id AS publicId, shops.name,
              CASE WHEN shops.hero_data IS NULL OR shops.hero_data = '' THEN NULL ELSE '/api/shops/' || shops.public_id || '/hero?v=' || COALESCE(shops.hero_hash, 'original') END AS heroUrl,
  ...
  return db
    .prepare(
      `SELECT machines.*, shops.name AS shop_name,
              CASE WHEN shops.hero_data IS NULL OR shops.hero_data = '' THEN NULL ELSE '/api/shops/' || shops.public_id || '/hero?v=' || COALESCE(shops.hero_hash, 'original') END AS shop_hero_url,
