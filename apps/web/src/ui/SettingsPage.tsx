        ...
        <section className="rounded border border-black/10 bg-panel p-5">
          <div className="flex items-center justify-between gap-3">
            ...
            {browserSupportsWebAuthn() && (
              <button className="focus-ring rounded bg-ink px-3 py-2 text-sm font-medium text-white disabled:opacity-60" disabled={busy} onClick={addPasskey}>
                添加 Passkey
              </button>
            )}
          </div>
            ...
            {passkeys.map((passkey) => (
              <div key={passkey.id} className="flex items-center justify-between gap-3 rounded border border-black/10 bg-white p-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-ink/60">{passkey.backedUp ? "已同步" : "此设备"}</p>
                </div>
                <button
